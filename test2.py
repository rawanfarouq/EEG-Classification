...
def extract_features_from_channel(channel_data, sfreq, epoch_length=1.0):
    ...
    for i in range(n_epochs):
        ...
        zcr_val = ((epoch[:-1] * epoch[1:]) < 0).sum() / epoch_length
        ...
        spectral_flatness = np.exp(np.mean(np.log(psd_vals+1e-10))) / np.mean(psd_vals) # Adding a small value to avoid log(0)
        spectral_centroid = np.sum(freq_res * psd_vals) / np.sum(psd_vals)
        ...
        features = {
            ...
            'entropy': entropy(psd_vals)
        }
        
        # Calculate power in each frequency band
        for band, (fmin, fmax) in bands.items():
            freq_inds = np.where((freq_res >= fmin) & (freq_res <= fmax))[0]
            band_power = psd_vals[freq_inds].sum()
            band_power_rel = band_power / psd_vals.sum()
            features[f'{band}_power'] = band_power
            features[f'{band}_power_rel'] = band_power_rel
        

        # Hjorth parameters - Activity, Mobility, Complexity
        activity = np.var(epoch)
        mobility = np.sqrt(np.var(np.diff(epoch)) / activity)
        complexity = np.sqrt(np.var(np.diff(np.diff(epoch))) / np.var(np.diff(epoch)) / mobility)
...
def main():
        ...
        if file_path.lower().endswith(('.csv', '.xls', '.xlsx', '.xlsm', '.xlsb')):
            ...
            if process_with_builtin_functions:
                ...
                if eeg_data is not None and labels is not None:
                    ...
                    # Make sure the data is in the right format for tsfresh
                    # Ensure the DataFrame is in the long format expected by tsfresh
                    # with 'id' as the time series identifier and 'time' as the time index.
    ...
    if process_with_builtin_functions:
        ...
        print(f'Overall model accuracy across all files: {overall_accuracy * 100:.2f}%')
